        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-3 font-medium text-gray-800 max-w-xs truncate">{{ $post->title }}</td>
                    <td class="px-6 py-3 text-gray-500">{{ $post->category ?? '-' }}</td>
                    <td class="px-6 py-3">
                        <span class="px-2 py-1 text-xs font-medium rounded-full {{ $post->is_published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                            {{ $post->is_published ? 'Publish' : 'Draft' }}
                        </span>
                    </td>
                    <td class="px-6 py-3 text-gray-500 text-xs">{{ $post->created_at->format('d M Y') }}</td>
                ...
                @empty
                <tr><td colspan="6" class="px-6 py-8 text-center text-gray-400">Belum ada berita.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div class="sm:hidden divide-y">
        ...
        <div class="p-4 flex gap-3">
            ...
            <div class="flex-grow min-w-0">
                ...
                <p class="text-xs text-gray-500 mt-0.5">{{ $post->category ?? '-' }} · {{ $post->created_at->format('d M Y') }}</p>
                <div class="flex items-center space-x-3 mt-2">
                    <span class="px-2 py-0.5 text-xs rounded-full {{ $post->is_published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">{{ $post->is_published ? 'Publish' : 'Draft' }}</span>
                    <a href="{{ route('admin.posts.edit', $post) }}" class="text-xs text-indigo-600">Edit</a>
