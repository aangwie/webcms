    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700 p-6">
        <form action="{{ route('admin.sliders.update', $slider) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $slider->title) }}" class="w-full rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500 px-4 py-2 border">
                @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Gambar <span class="text-gray-400 dark:text-slate-500 text-xs">(maks 800KB, kosongkan jika tidak ganti)</span></label>
                @if($slider->image_path)
                <div class="mb-3">
                    <img src="{{ asset($slider->image_path) }}" alt="{{ $slider->title }}" class="h-32 w-full object-cover rounded-lg border dark:border-slate-600">
                    <span class="text-xs text-gray-500 dark:text-slate-400 mt-1 block">Gambar saat ini
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="order_index" value="{{ old('order_index', $slider->order_index) }}" class="w-full rounded-lg border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white shadow-sm focus:ring-indigo-500 focus:border-indigo-500 px-4 py-2 border">
                </div>
                <div class="flex items-end pb-1">
                    <label class="flex items-center space-x-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ $slider->is_active ? 'checked' : '' }} class="rounded border-gray-300 dark:border-slate-600 text-indigo-600 focus:ring-indigo-500">
