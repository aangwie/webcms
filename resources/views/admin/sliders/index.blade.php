...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-gray-500">Kelola gambar hero carousel di halaman utama. <span class="text-amber-600 font-medium">Maks 800KB per gambar.</span></p>
    ...
    <div class="bg-white rounded-xl shadow-sm border overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-left">
                <tr>
                    <th class="px-4 py-3 font-medium text-gray-600 w-16">#</th>
                    <th class="px-4 py-3 font-medium text-gray-600">Gambar</th>
                    <th class="px-4 py-3 font-medium text-gray-600">Judul</th>
                    <th class="px-4 py-3 font-medium text-gray-600 w-20">Urutan</th>
                    <th class="px-4 py-3 font-medium text-gray-600 w-20">Status</th>
                    <th class="px-4 py-3 font-medium text-gray-600 w-28">Ukuran</th>
                    <th class="px-4 py-3 font-medium text-gray-600 w-32 text-right">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">
                        <img src="{{ asset($slider->image_path) }}" alt="{{ $slider->title }}" class="h-16 w-28 object-cover rounded-lg border">
                    </td>
                    <td class="px-4 py-3 text-gray-800 font-medium">{{ $slider->title ?? '-' }}</td>
                    <td class="px-4 py-3 text-gray-600">{{ $slider->order_index }}</td>
                    <td class="px-4 py-3">
                        @if($slider->is_active)
                    ...
                    <td class="px-4 py-3 text-right space-x-2">
                        <a href="{{ route('admin.sliders.edit', $slider) }}" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium">Edit</a>
    ...
    <div class="bg-white rounded-xl shadow-sm border p-12 text-center">
        ...
        <p class="text-gray-500 mb-4">Belum ada slider hero carousel.</p>
